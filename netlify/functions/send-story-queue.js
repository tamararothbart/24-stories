...
exports.handler = async function() {
  ...
  // --- COACHING EMAIL 0 CHECK ---
  // Fires between welcome email and first Wednesday prompt.
  // Timing bands (based on hours remaining until first prompt):
  //   120h+ (5+ days)  → fire 24h after welcome
  //   48–120h (2–5d)   → fire 6h after welcome
  //   <48h (<2d)       → fire 2h after welcome
  // Safety net: PromptNumber = 0 ensures this never fires after first prompt dispatches.

  const c0Formula = encodeURIComponent(
    'AND({Status}="Active",{CoachingEmailsSent}=0,NOT({InCoaching}),{WelcomeEmailSentAt}!="",{PromptNumber}=0)'
  );
  const c0Res = await fetch(
    `https://api.airtable.com/v0/${BASE}/Subscribers?filterByFormula=${c0Formula}&maxRecords=20`,
    { headers: hdrs }
  );
  if (c0Res.ok) {
    const { records: c0Subs } = await c0Res.json();
    for (const sub of c0Subs) {
      const f = sub.fields;
      if (!f.StorytellerEmail || !f.WelcomeEmailSentAt) continue;
      ...
      // Mark sent first — prevents double-send on next 2-minute cycle
      await fetch(`https://api.airtable.com/v0/${BASE}/Subscribers/${sub.id}`, {
        method: 'PATCH',
        headers: { 'Authorization': `Bearer ${PAT}`, 'Content-Type': 'application/json' },
      ...
      console.log(`Coaching email 0 sent to ${f.StorytellerEmail}`);
    }
  }
...
// Returns hours until next Wednesday 7am SAST (5am UTC) from the given timestamp.
// Used by the coaching email 0 timing logic.
function hoursUntilNextWednesdayPrompt(nowMs) {
  const now        = new Date(nowMs);
  const dayUTC     = now.getUTCDay();
  const hourUTC    = now.getUTCHours();
  const promptUTC  = 5; // 7am SAST = 5am UTC

  let daysToAdd;
  if      (dayUTC < 3)                          daysToAdd = 3 - dayUTC;
  else if (dayUTC === 3 && hourUTC < promptUTC) daysToAdd = 0; // today's prompt not yet sent
  else if (dayUTC === 3)                         daysToAdd = 7; // today's prompt already sent
  else                                           daysToAdd = (3 + 7) - dayUTC;

  const next = new Date(now);
  next.setUTCDate(next.getUTCDate() + daysToAdd);
  next.setUTCHours(promptUTC, 0, 0, 0);
...
<div style="background:#F7F5F2;padding:48px 40px;color:#1A1A1A;">
  <img src="https://resilient-eclair-c46b34.netlify.app/logo.png" alt="24 Stories" width="180" height="40" style="display:block;border:0;max-width:100%;height:auto;margin-bottom:36px;margin-left:auto;">
  <p style="font-size:17px;line-height:1.9;margin:0 0 22px;">Hello ${esc(firstName)},</p>
  <p style="font-size:17px;line-height:1.9;margin:0 0 22px;">Welcome. Your prompts start soon — but before they do, I wanted to send you this.</p>
  ...
  <a href="https://wa.me/27823758320" style="display:inline-block;background:#1A1A1A;color:#fff;text-decoration:none;padding:16px 36px;font-size:16px;letter-spacing:0.05em;margin:0 0 40px;">WhatsApp Tamara &#8594;</a>
  <hr style="border:none;border-top:1px solid #D0CCC6;margin:36px 0;">
